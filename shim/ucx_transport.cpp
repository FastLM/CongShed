...
int UcxTransport::zcopy_put(UcxEndpoint& ep, const void* buf, size_t len,
    ...
    req.transfer_id = shim_.submit_transfer(cls, ep.local, ep.remote, len, dl);
    req.length = len;
    req.completed = false;
...
int UcxTransport::zcopy_get(UcxEndpoint& ep, void* buf, size_t len, UcxRequest& req,
    ...
    (void)buf;
    // Get is modeled as reverse-direction put for scheduling purposes
    UcxEndpoint rev{ep.remote, ep.local, ep.uct_ep};
    return zcopy_put(rev, nullptr, len, req, cls, deadline_us);
}
